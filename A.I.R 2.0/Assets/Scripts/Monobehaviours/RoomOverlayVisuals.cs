...
public class RoomOverlayVisuals : MonoBehaviour
{
    ...
    public void SetRoomOverlay(RoomGrid lvRoomGrid)
    {
        
        roomGrid = lvRoomGrid.roomGrid;
        ...
        roomGrid.OnGridValueChanged += TileGrid_OnGridValueChanged;
    ...
    public void UpdateRoomOverlayVisual()
    {
        ...
        MeshUtilities.CreateEmptyMeshArray(roomGrid.width * roomGrid.height, out Vector3[] vertices, out Vector2[] uv, out int[] triangles);
        for (int x = 0; x < roomGrid.width; x++)
        {
            ...
            for (int y = roomGrid.height - 1; y > -1; y--)
            {
                ...
                MeshUtilities.AddToMeshArrays(vertices, uv, triangles, index, new Vector3(roomGrid.GetWorldPosition(x, y).x + quadsize.x * 0.5f, roomGrid.GetWorldPosition(x, y).y), 0f, quadsize, gridUV00, gridUV11);
            }
        }
    ...
    private void TileGrid_OnGridValueChanged(object sender, Grid<RoomGridObject>.OnGridValueChangedEvent e)
    {
        //Debug.Log("Room Visuals Reset");
        updateOverlayMesh = true;
    }
    ...
    private void TileMap_OnRoomCreated(object sender, RoomGrid.OnRoomCreatedEvent e)
    {
        Debug.Log("Room Visuals Reset");
        rooms.Add(e.newRoom);
        /*int roomNo = rooms.Count - 1;
        List<UVCoords> uVCoords = new List<UVCoords>();
        if (roomNo <= roomOverlaySpriteUVs.Length)
        {
            UVCoords newUVCoords = new UVCoords
            {
                uv00 = new Vector2(roomOverlaySpriteUVs[roomNo].uv00pixels.x / textureW, roomOverlaySpriteUVs[roomNo].uv00pixels.y / textureH),
                uv11 = new Vector2(roomOverlaySpriteUVs[roomNo].uv11pixels.x / textureW, roomOverlaySpriteUVs[roomNo].uv11pixels.y / textureH),
            };
            uvCoordsDictionary.Add(e.newRoom, newUVCoords);
            uVCoords.Add(newUVCoords);
        }
        else
        {
            uvCoordsDictionary.Add(e.newRoom, uVCoords[uVCoords.Count - 1]);
        }*/
